...
def _emit_cpp(defs: LogDefs, base_type: str, out_path: Path) -> int:
    ...
    w("")
    w("struct tCapFlag {")
    w("    uint8_t byte;  // 1-based index into the 96 cap-flag bytes; 0 == not gated")
    w("    uint8_t bit;   // 1-based bit within that byte (1..8);     0 == not gated")
    w("")
    ...
    w("    eStorageType     storage;    // wire encoding")
    w("    uint8_t          decimals;   // display precision hint")
    ...
    for p in params:
        ...
        storage = f"eStorageType::{_STORAGE_MAP.get(p.storagetype or '', 'Unknown')}"
