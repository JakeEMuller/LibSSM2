...
public:
    IsoTpTransport(ICanBus* bus, uint32_t reqId, uint32_t respId) noexcept;
    ...
    [[nodiscard]] eStatus SendRequest(const uint8_t* payload, size_t payloadLen,
                                       uint32_t timeoutMs);
    ...
    [[nodiscard]] eStatus ReceiveResponse(uint8_t* out, size_t outCapacity,
                                           size_t* outLen,
                                           uint32_t timeoutMs);

    // Convenience: send + receive. The deadline applies to the whole round-
    // trip, not each phase individually.
    [[nodiscard]] eStatus Exchange(const uint8_t* req, size_t reqLen,
