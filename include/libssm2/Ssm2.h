...
public:
    ...
    [[nodiscard]] eStatus Init(tSsm2InitResponse* out,
    ...
    [[nodiscard]] eStatus ReadAddresses(const uint32_t* addrs,
                                         size_t addrCount,
                                         uint8_t* out,
                                         uint32_t timeoutMs = 0);
    ...
    // at startAddr.
    [[nodiscard]] eStatus ReadBlock(uint32_t startAddr,
                                     uint8_t* out, size_t outLen,
                                     uint32_t timeoutMs = 0);

    // eSsm2Cmd::WriteAddress - write a single byte to one address.
    [[nodiscard]] eStatus WriteAddress(uint32_t addr, uint8_t value,
                                        uint32_t timeoutMs = 0);

    // eSsm2Cmd::WriteBlock - sequential block write of `dataLen` bytes
    // starting at startAddr.
    [[nodiscard]] eStatus WriteBlock(uint32_t startAddr,
    ...
    [[nodiscard]] eStatus StartContinuous(const uint32_t* addrs,
                                           size_t addrCount);

    [[nodiscard]] eStatus PollContinuous(uint8_t* out, size_t outCapacity,
                                          size_t* outRecords,
                                          uint32_t timeoutMs);
